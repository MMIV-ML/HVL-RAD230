HVL-RAD230 Setup for the Imaging and Artificial Intelligence course module


# Setting up your system for medical image processing and analysis
...
In Colab (in `0-test-installation.ipynb`), you might need to force matplotlib version:
```
...
```bash
git clone https://github.com/MMIV-ML/HVL-RAD230
cd HVL-RAD230
```
...
Go through the notebook [`0-test-installation.ipynb`](https://nbviewer.org/github/MMIV-ML/HVL-RAD230/blob/master/0-test-installation.ipynb) in the local `HVL-RAD230` repository directory:
```bash
jupyter notebook  (or, jupyter lab)
```

## Update:
The image processing and analysis code and environment will likely be updated during the course. Run the following commands regularly from your locally cloned repo in folder `HVL-RAD230`:
...
fatal: iconv_open(UTF-8,UTF-8-MAC) failed, but needed:
    ...
    If you want to use decomposed unicode,
```
